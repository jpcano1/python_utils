...
class RecurrentUNet(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        down_block = RecurrentDownBlock(current_filters, current_filters * 2, *args, **kwargs)
        ...
        for _ in range(depth - 1):
            # Create the recurrent up block
            up_block = RecurrentUpBlock(
    ...
    def forward(self, x):
        ...
        for idx, block in enumerate(self.down_blocks):
            x = block(x)
            if isinstance(block, RecurrentDownBlock):
                down_blocks.append(x)

        # Begin to forward pass the x tensor up
        # with the recurrent down blocks concatenated
