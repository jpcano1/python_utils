...
def setup_workshop_9(filename: str = "kaggle.json", download_dataset=True, kaggle_version="1.5.6"):
    setup_general.setup_general()
...
def setup_workshop_13(download_dataset=True, pretrained=True, brats=False):
    ...
    if pretrained:
        ...
        gen.download_file_from_google_drive(unet_id, "unet.pt", dst="./models", size=22e3)
        print("Pretrained Networks Downloaded Successfully")
    if brats:
        part_1 = "1NRAFehzp6WtpoQduMJ5PGt1ivC8NhpJ1"
        part_2 = "1-6vfu9nBu1PIabcZPMAmX685M1mOkceI"
        gen.download_file_from_google_drive(part_1, "brats_part_1.zip", size=426.8e3)
        gen.download_file_from_google_drive(part_2, "brats_part_2.zip", size=370.3e3)
