...
def get_labeled_image(img, label, num_classes, as_categorical=True):
    if as_categorical:
        aux_label = keras.utils.to_categorical(label, num_classes=num_classes, dtype="uint8")
    else:
...
def create_data(
    dataset, bg_thresh=0.97, save_dir="/content/train_data", sanity_check=False, test=False
):
